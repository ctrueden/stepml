...
def analyze_performance_correlation(df):
    ...
    perf_data = df[df['has_performance_data'] == True].copy()
    ...
    perf_cols = [
        'times_played', 'best_accuracy', 'average_accuracy',
        'perceived_difficulty_factor', 'perfect_rate', 'miss_rate'
    ]
    ...
    rating_groups = perf_data[perf_data['best_accuracy'].notna()].groupby('original_rating')
...
def find_rating_anomalies(df):
    ...
    print()
    print(f"{'Title':<40} {'Diff':<10} {'Rating':>6} {'Accuracy':>9} {'Avg@Rating':>11} {'Delta':>7} {'Plays':>6}")
    print("-" * 110)
    ...
    for chart in easier_charts[:15]:
        print(f"{chart['title']:<40} {chart['difficulty']:<10} {chart['rating']:6.0f} "
...
def performance_feature_summary(df):
    ...
    perf_data = df[df['has_performance_data'] == True]

    print(f"\nTotal charts in dataset: {len(df)}")
    print(f"Charts with performance data: {len(perf_data)} ({len(perf_data)/len(df)*100:.1f}%)")
