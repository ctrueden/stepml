...
def test_dance_solo_parsing():
    ...
    assert len(solo_charts) == 3, (
        f"Expected 3 dance-solo charts, found {len(solo_charts)}"
    )

    # Check each chart has notes
    for chart in solo_charts:
        print(
            f"  {chart.difficulty.value} (rating {chart.rating}): {chart.total_notes} notes"
        )
        assert chart.total_notes > 0, (
            f"{chart.difficulty.value} chart has 0 notes (parsing failed)"
        )

        # Verify 6-column format
        if chart.note_positions:
            first_note = chart.note_positions[0][1]
            assert len(first_note) == 6, (
                f"Expected 6 columns for solo chart, got {len(first_note)}"
            )

    # Verify specific note counts (approximate ranges)
    easy_chart = [c for c in solo_charts if c.difficulty.value == "Easy"][0]
    assert 140 <= easy_chart.total_notes <= 160, (
        f"Easy chart notes out of range: {easy_chart.total_notes}"
    )

    medium_chart = [c for c in solo_charts if c.difficulty.value == "Medium"][0]
    assert 210 <= medium_chart.total_notes <= 230, (
        f"Medium chart notes out of range: {medium_chart.total_notes}"
    )

    hard_chart = [c for c in solo_charts if c.difficulty.value == "Hard"][0]
    assert 310 <= hard_chart.total_notes <= 340, (
        f"Hard chart notes out of range: {hard_chart.total_notes}"
    )
