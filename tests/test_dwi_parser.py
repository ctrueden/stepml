...
def test_zip_a_dee_doo_dah():
    ...
    dwi = parse_dwi_file(
        "../Songs/DS feat. Disney's Rave/Zip-A-Dee-Doo-Dah/Zip-A-Dee-Doo-Dah.dwi"
    )
    ...
    single_basic = [
        c
        for c in dwi.charts
        if c.chart_type.value == "dance-single" and c.difficulty.value == "Easy"
    ][0]
    print(f"SINGLE Easy: {single_basic.total_notes} notes")
    assert 85 <= single_basic.total_notes <= 95, (
        f"Expected ~89 notes, got {single_basic.total_notes}"
    )
    ...
    double_basic = [
        c
        for c in dwi.charts
        if c.chart_type.value == "dance-double" and c.difficulty.value == "Easy"
    ][0]
    print(f"DOUBLE Easy: {double_basic.total_notes} notes")
    assert 110 <= double_basic.total_notes <= 130, (
        f"Expected ~120 notes, got {double_basic.total_notes}"
    )

    # Verify left and right notes are distributed
    left_notes = sum(
        1 for _, notes in double_basic.note_positions if notes[:4].count("1") > 0
    )
    right_notes = sum(
        1 for _, notes in double_basic.note_positions if notes[4:].count("1") > 0
    )
...
def test_castlevania():
    ...
    assert 290 <= basic.total_notes <= 320, (
        f"Expected ~306 notes, got {basic.total_notes}"
    )

    # Test ANOTHER
    another = [c for c in dwi.charts if c.difficulty.value == "Medium"][0]
    print(f"SINGLE Medium (ANOTHER): {another.total_notes} notes")
    assert 380 <= another.total_notes <= 410, (
        f"Expected ~397 notes, got {another.total_notes}"
    )
    ...
    assert 490 <= maniac.total_notes <= 520, (
        f"Expected ~509 notes, got {maniac.total_notes}"
    )
    ...
    for chart in dwi.charts:
        if chart.chart_type.value == "dance-double":
            ...
            for beat, notes in chart.note_positions:
                assert len(notes) == 8, (
                    f"DOUBLE chart should have 8-column notes, got {len(notes)}"
                )
...
def test_single_format():
    ...
    for chart in dwi.charts:
        ...
        for beat, notes in chart.note_positions[:5]:  # Check first 5
            assert len(notes) == 4, (
                f"SINGLE chart should have 4-column notes, got {len(notes)}"
            )

    print("✓ SINGLE charts use proper 4-column format")
