...
def test_dwi_parser():
    """Test DWI parser on a real file."""
    # Test file path
    dwi_file = "/home/curtis/Games/StepMania/Songs/DDR GB 2/PARANOiA Rebirth/PARANOiA Rebirth.dwi"

    print(f"Testing DWI parser on: {dwi_file}\n")
    print("=" * 80)

    # Parse the file
    chart_data = parse_dwi_file(dwi_file)
    ...
    print("TIMING")
    print("-" * 80)
    print(f"Offset: {chart_data.offset:.3f}s (converted from GAP)")
    print(f"BPMs: {len(chart_data.bpms)} value(s)")
    if chart_data.bpms:
        for bpm in chart_data.bpms:
    ...
    for chart in chart_data.charts:
        ...
        for i, (beat, notes) in enumerate(chart.note_positions[:5]):
            print(f"    Beat {beat:.3f}: {notes}")
        print()

    print("=" * 80)
    print("DWI PARSER TEST COMPLETE!")
    print("=" * 80)
