...
def generate_ratings(dataset_path: Path, model_dir: Path, output_path: Path):
    ...
    print("\nCalculated ratings:")
    print(
        f"  Range: {df['calculated_rating'].min():.2f} - {df['calculated_rating'].max():.2f}"
    )
    print(
        f"  Mean: {df['calculated_rating'].mean():.2f} ± {df['calculated_rating'].std():.2f}"
    )
    ...
    if stats_path.exists():
        ...
        if warnings:
            ...
            for w in warnings:
                ...
                print(
                    f"{short_file:<50} {w['chart_type']:<14} {w['difficulty']:<10}"
...
def main():
    ...
    generate_ratings(args.dataset, args.model_dir, args.output)
    ...
    print(
        f"  Full dataset: {args.output.parent / 'dataset_with_calculated_ratings.parquet'}"
    )
