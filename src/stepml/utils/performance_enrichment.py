...
from stepml.parsers.stats_parser import StatsParser
...
class PerformanceEnricher:
    ...
    def get_performance_features(
        self, file_path: str, difficulty: str, chart_type: str
    ) -> Dict[str, Any]:
        """
        ...
        perf = self.stats_parser.get_performance(
            song_dir, norm_difficulty, norm_chart_type
        )
        ...
        if perf.best_accuracy is not None:
            ...
            if perf.best_accuracy > 0.01:  # Avoid division by zero
                ...
                features["perceived_difficulty_factor"] = max(
                    0.5, min(3.0, perceived_factor)
                )
            else:
        ...
        if perf.best_tap_scores:
            ...
            if total_notes > 0:
                ...
                features["great_or_worse_rate"] = (
                    ts.w3 + ts.w4 + ts.w5 + ts.miss
                ) / total_notes
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_performances_in_stats": len(
                self.stats_parser.get_all_performances()
            ),
        }
