...
class GroundTruthOverrides:
    ...
    def _load(self):
        ...
        logger.info(f"Loaded {len(self.overrides)} ground truth override(s) from {self.path}")
    ...
    def apply(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for override in self.overrides:
            ...
            if n == 0:
                ...
                logger.info(
                    ...
                    repr(title), chart_type, difficulty, rating, n, "s" if n != 1 else ""
                )
        ...
        if n_overridden:
            logger.info(
                ...
                n_overridden, "s" if n_overridden != 1 else "", len(df) - n_overridden
            )
        return df
